...
export default function Toolbar() {
    ...
    const [_pdfContent, setPdfContent] = useState<string>("");
    const [_variants, setVariants] = useState<{ [key: string]: string }>({});
    ...
    const generateHtmlVariants = async (
        ...
        try {
            ...
            logger.error({ error }, "Error generating variants");
        } finally {
    ...
    const ensureValidHtml = (htmlInput: string): string => {
        let html = htmlInput;
        ...
        if (!html.includes("<head>") && !html.includes("<head ")) {
            ...
            if (htmlEndIndex !== -1) {
                ...
                html = `${html.slice(0, insertPosition)}<head><title>Generated Website</title></head>${html.slice(insertPosition)}`;
            }
        }
        ...
        if (!html.includes("<body>") && !html.includes("<body ")) {
            ...
            if (headEndIndex !== -1) {
                ...
                html = `${html.slice(0, insertPosition)}<body>${html.slice(insertPosition)}</body>`;
            } else {
                // If no </head>, insert before </html>
                const htmlEndIndex = html.indexOf("</html>");
                if (htmlEndIndex !== -1) {
                    html = `${html.slice(0, htmlEndIndex)}<body></body>${html.slice(htmlEndIndex)}`;
                }
            }
        }
            ...
            {step === "generating" && (
                <div className="p-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            Portfolio {generationProgress.portfolio ? "✓" : ""}
                        </Button>
                        <Button
                            variant="outline"
                            size="sm"
