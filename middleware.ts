...
import { apiRatelimit, ratelimit } from "./libs/utils/api/ratelimit";
...
// Function to check for public routes (add your excluded routes here)
const isPublicRoute = (pathname: string) => {
  ...
  // Dynamic matches (e.g., /resume/some-id)
...
export async function middleware(request: NextRequest) {
  ...
  const pathname = request.nextUrl.pathname; // Capture pathname once

  try {
    const sessionResponse = await updateSession(request, NextResponse.next(), {
    ...
    // If there is no user, and the current route is NOT a public route, redirect to login
    if (
      !user &&
      !isPublicRoute(pathname) &&
      !isAuthCallback // This handles all callback formats you defined earlier
    ) {
...
export const config = {
  // Your existing robust matcher remains the same
  matcher: [
